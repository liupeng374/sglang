...
class DSV4NPUTokenToKVPool(DeepSeekV4TokenToKVPool):
    ...
    def _make_indexer_pool(
        ...
        if force_fp4:
            # DSV4.1 low compress ratios: the fp4 rounding lives in the model
            # (rope + fake_quant_fp4 before the write), so NPU stores the
            # already-grid-aligned values as bf16 instead of packed fp4.
            return NPULowRatioIndexerPool(
                size,
                page_size,
                dtype,
                index_head_dim,
